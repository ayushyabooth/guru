...
export const DiveinFeed: React.FC<DiveinFeedProps> = ({
  ...
  // Normalize a string for fuzzy industry matching: lowercase, strip spaces/underscores/ampersands/dashes
  const normalizeIndustry = (s: string) => s.toLowerCase().replace(/[\s_&\-]+/g, '');

  // Extract raw industry key from filterContext (e.g. 'interest:food_beverage' → 'food_beverage')
  const filterContextKey = filterContext.includes(':') ? filterContext.split(':')[1] : filterContext;

  // Apply client-side filter for every pill except 'All' and 'core'.
  // 'core' relies on the API to return the right articles for the user's primary industry.
  // Specialization/interest pills may receive a broader result set and need client-side narrowing.
  ...
  const visibleArticles = needsClientFilter
    ? articles.filter(a => {
        const articleIndustry = normalizeIndustry(a.industry || a.context || '');
        const byLabel = normalizeIndustry(filterLabel!);
        const byKey = normalizeIndustry(filterContextKey);
        // Match by display label OR by the raw context key (handles snake_case vs display name mismatches)
        return (
          articleIndustry === byLabel ||
          articleIndustry.includes(byLabel) ||
          byLabel.includes(articleIndustry) ||
          articleIndustry === byKey ||
          articleIndustry.includes(byKey) ||
          byKey.includes(articleIndustry)
        );
      })
    : articles;
